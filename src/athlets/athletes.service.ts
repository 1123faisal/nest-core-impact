...
export class AthletesService {
  ...
  async find(skip?: number, limit?: number): Promise<PaginatedDto<User>> {
    return {
      results: await this.userModel
        // .find({ physician_coach: null })
        .find({ physician_coach: { $ne: null, $exists: true } })
        .find({ batting_coach: { $ne: null, $exists: true } })
        .find({ trainer_coach: { $ne: null, $exists: true } })
        .find({ pitching_coach: { $ne: null, $exists: true } })
        .sort({ _id: -1 })
        .skip(skip)
        .limit(limit)
        .populate({
          path: 'physicia n_coach',
          select: { name: 1 },
        })
        .populate({
          path: 'batting_coach',
  ...
  // async removechoachs(id: string) {
  //   return await this.userModel.findByIdAndDelete(id);
  // }
  // async unassignCoaches(id: string, coachIds: string[]) {
  //   await this.findOne(id);
  //   return await this.userModel.findByIdAndUpdate(
  //     id,
  //     {
  //       $pullAll: { caoch: { $in: coachIds } },
  //     },
  //     { new: true },
  //   );
  // }
