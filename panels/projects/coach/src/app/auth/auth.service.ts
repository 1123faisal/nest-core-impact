...
export class AuthService {
  private tokenKey = 'coach_authData';
  ...
  signUp(name: string, email: string, password: string) {
    return this.http
      .post<AuthData>(`${environment.apiUri}/api/v1/coachs/signup`, {
        password,
        name,
        email,
      })
  ...
  signIn(email: string, password: string) {
    return this.http
      .post<AuthData>(`${environment.apiUri}/api/v1/coachs/login`, {
        password,
        email,
      })
      .pipe(
  ...
  forgotPassword(email: string) {
    return this.http.post(
      `${environment.apiUri}/api/v1/coachs/forget-password/send-otp`,
      {
        email,
      }
    );
  ...
  verifyOtp(email: string, otp: string) {
    return this.http.post(
      `${environment.apiUri}/api/v1/coachs/forget-password/verify-otp`,
      {
        email,
        otp,
      }
  ...
  resentOtp(email: string) {
    return this.http.post(`${environment.apiUri}/api/v1/coachs/resend-otp`, {
      email,
    });
  }
  ...
  resetPassword(
    ...
    return this.http.post(
      `${environment.apiUri}/api/v1/coachs/forget-password/update-password`,
      {
        password,
        email,
        otp,
