...
export class AuthService {
  private tokenKey = 'admin_authData';
  ...
  signUp(name: string, email: string, password: string) {
    return this.http
      .post<AuthData>(`${environment.apiUri}/api/v1/admins/signup`, {
        password,
        name,
        email,
      })
  ...
  signIn(email: string, password: string) {
    return this.http
      .post<AuthData>(`${environment.apiUri}/api/v1/admins/login`, {
        password,
        email,
      })
      .pipe(
  ...
  forgotPassword(email: string) {
    return this.http.post(
      `${environment.apiUri}/api/v1/admins/forget-password/send-otp`,
      {
        email,
      }
    );
  ...
  verifyOtp(email: string, otp: string) {
    return this.http.post(
      `${environment.apiUri}/api/v1/admins/forget-password/verify-otp`,
      {
        email,
        otp,
      }
  ...
  resentOtp(email: string) {
    return this.http.post(`${environment.apiUri}/api/v1/admins/resend-otp`, {
      email,
    });
  }
  ...
  resetPassword(
    ...
    return this.http.post(
      `${environment.apiUri}/api/v1/admins/forget-password/update-password`,
      {
        password,
        email,
        otp,
